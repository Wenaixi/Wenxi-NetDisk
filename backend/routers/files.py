...
async def upload_file(
    ...
    try:
        ...
        async with aiofiles.open(temp_path, "wb") as buffer:
            while True:
                ...
                if chunk_count % 20 == 0:
                    ...
                    logger.debug(
                        f"Wenxi - 上传进度: {file.filename} - {file_size / 1024 / 1024:.2f}MB ({speed:.2f}MB/s)"
                    )
        ...
        encrypt_success = encrypt_file(
            temp_path, file_path, user_id=current_user.id, file_id=db_file.id
        )
        ...
        logger.info(
            f"Wenxi - 文件上传完成: {file.filename} ({file_size} bytes, {upload_speed:.2f}MB/s)"
        )
...
async def upload_chunk(
    ...
    try:
        ...
        actual_hash = await loop.run_in_executor(
            executor, calculate_file_hash, chunk_path
        )
...
async def merge_chunks(
    ...
    try:
        ...
        encrypt_success = encrypt_file(
            final_path, encrypted_path, user_id=current_user.id, file_id=db_file.id
        )
        ...
        logger.info(
            f"Wenxi - 分块上传完成: {file_name} ({file_size} bytes, {upload_speed:.2f}MB/s)"
        )
...
async def list_files(
    ...
    try:
        ...
        if search:
            ...
            query = query.filter(FileModel.original_filename.ilike(search_term))
...
async def download_file(
    ...
    try:
            ...
            if cached_meta:
                ...
                if file:
                    await redis.setex(
                        f"file:meta:{file_id}",
                        CACHE_TTL,
                        str({"filename": file.original_filename}),
                    )
        except:
            file = (
                db.query(FileModel)
        ...
        logger.info(
            f"Wenxi - 尝试下载文件: {file.original_filename}, 路径: {file_path}"
        )
        if not os.path.exists(file_path):
            logger.error(f"Wenxi - 文件不存在: {file_path}")
            logger.error(f"Wenxi - 文件ID: {file_id}, 用户ID: {current_user.id}")
            raise HTTPException(
                status_code=404, detail=f"文件不存在: {file.original_filename}"
            )
        ...
        decrypt_success = decrypt_file(
            file_path, temp_decrypt_path, user_id=file.owner_id, file_id=file.id
        )
        ...
        if not decrypt_success:
            ...
            logger.error(
                f"Wenxi - 文件解密失败 - 文件ID: {file_id}, 用户ID: {file.owner_id}, 文件路径: {file_path}"
            )
            raise HTTPException(
                status_code=500, detail=f"文件解密失败: {file.original_filename}"
            )
...
async def access_shared_file(
    ...
    try:
        file = (
            db.query(FileModel)
            .filter(FileModel.share_token == share_token, FileModel.is_shared == True)
            .first()
        )
        ...
        file_path = os.path.abspath(file_path)
        logger.info(
            f"Wenxi - 通过分享链接访问文件: {file.original_filename}, 路径: {file_path}"
        )
...
async def delete_file(
    ...
    try:
        ...
        # 检查文件是否已在垃圾桶
        if file.is_deleted:
            raise HTTPException(status_code=400, detail="文件已在垃圾桶中")

        # 保存原始路径用于恢复
        file.original_path = file.file_path
        ...
        logger.info(
            f"用户 {current_user.username} 将文件移至垃圾桶: {file.original_filename}"
        )

        return {"message": "文件已移至垃圾桶", "file_id": file_id}
...
async def merge_chunks(upload_id: str, temp_dir: str, current_user: User, db: Session):
    ...
    try:
        ...
        final_path = os.path.join(
            os.path.dirname(__file__), "..", "uploads", final_filename
        )
        ...
        logger.info(
            f"Wenxi - 分片合并完成: upload_id={upload_id}, filename={final_filename}"
        )
...
async def share_file(
    ...
    try:
        ...
        return FileShareResponse(
            share_url=f"/api/files/shared/{share_token}", share_token=share_token
        )
