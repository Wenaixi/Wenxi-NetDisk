...
async def get_current_user(
    ...
    try:
        ...
        if datetime.fromtimestamp(exp, tz=timezone.utc) < datetime.now(timezone.utc):
            logger.warning(
                f"JWT令牌已过期: {datetime.fromtimestamp(exp, tz=timezone.utc)}"
            )
            raise credentials_exception
...
async def get_current_user_from_token(token: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if datetime.fromtimestamp(exp, tz=timezone.utc) < datetime.now(timezone.utc):
            logger.warning(
                f"iframe下载：JWT令牌已过期: {datetime.fromtimestamp(exp, tz=timezone.utc)}"
            )
            raise credentials_exception
...
async def login(
    ...
    access_token = create_access_token(
        data={"sub": user.username, "remember_me": remember_me},
...
async def delete_account(
    ...
    if request.username != current_user.username:
        ...
        raise HTTPException(status_code=400, detail="用户名不正确")

    # 验证邮箱
    if request.email != current_user.email:
        logger.warning(f"邮箱验证失败: {request.email} != {current_user.email}")
        raise HTTPException(status_code=400, detail="邮箱不正确")
    ...
    try:
        # 获取用户的所有文件
        user_files = (
            db.query(FileModel).filter(FileModel.owner_id == current_user.id).all()
        )
        ...
        raise HTTPException(status_code=500, detail="删除账户时发生错误")
